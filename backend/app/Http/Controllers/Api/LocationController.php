...
class LocationController extends Controller
{
    ...
    public function airportSearch(Request $request): JsonResponse
    {
        ...
        $airports = Airport::query()
            ...
            ->orderByRaw(
                'CASE WHEN iata_code = ? THEN 0 WHEN icao_code = ? THEN 1 WHEN name LIKE ? THEN 2 ELSE 3 END',
                [strtoupper($q), strtoupper($q), $q . '%']
            )
            ->orderBy('name')
            ->limit($limit)
            ->get();
    ...
    public function show(Location $location): JsonResponse
    {
        $location->load([
            ...
            'city',
            'airport.terminals',
            'points' => fn ($q) => $q->where('is_active', true)->where('is_public', true)->orderBy('sort_order')->orderBy('name'),
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $location = DB::transaction(function () use ($data) {
            ...
            if (($data['type_code'] ?? null) === LocationType::AIRPORT) {
                $airport = $this->resolveAirport($data);
                $this->syncTerminals($airport, $data['terminals'] ?? []);
            }
        ...
        return response()->json([
            ...
            'data' => $location->load(['type', 'airport.terminals']),
        ], 201);
    }
    ...
    public function update(Request $request, Location $location): JsonResponse
    {
        ...
        DB::transaction(function () use ($data, $location) {
            ...
            if (($data['type_code'] ?? null) === LocationType::AIRPORT) {
                $airport = $airport ?: $this->resolveAirport($data);
                $airport->update($this->airportPayload($data, $airport));
                $this->syncTerminals($airport, $data['terminals'] ?? []);
            }

            $location->update($this->locationPayload($data, $airport?->id));
        });

        return response()->json([
            'message' => 'Location updated.',
            'data' => $location->fresh()->load(['type', 'airport.terminals']),
        ]);
    }
    ...
    public function storePoint(Request $request, Location $location): JsonResponse
    {
        ...
        $point = $location->points()->create(array_merge($data, [
            'is_public' => true,
            'is_active' => true,
            'sort_order' => 0,
        ]));
        ...
        return response()->json([
            'message' => 'Location point created.',
            'data' => $point->load('airportTerminal'),
        ], 201);
    }
    ...
    public function points(Request $request, Location $location): JsonResponse
    {
        ...
        if (!empty($validated['point_type'])) {
            $query->where('point_type', $validated['point_type']);
        }
        if (filter_var($validated['pickup_only'] ?? false, FILTER_VALIDATE_BOOL)) {
            $query->where('is_pickup_allowed', true);
        }
        if (filter_var($validated['dropoff_only'] ?? false, FILTER_VALIDATE_BOOL)) {
            $query->where('is_dropoff_allowed', true);
        }
    ...
    private function validateLocation(Request $request, ?Location $location = null): array
    {
        $data = $request->validate([
            ...
            'native_name' => ['nullable', 'string', 'max:180'],
            'code' => ['nullable', 'string', 'max:50'],
            ...
            'terminals' => ['nullable', 'array', 'max:20'],
            'terminals.*.name' => ['required_with:terminals', 'string', 'max:100'],
            'terminals.*.code' => ['nullable', 'string', 'max:30'],
            'terminals.*.type' => ['nullable', 'string', 'max:50'],
        ]);
    ...
    private function resolveAirport(array $data): Airport
    {
        ...
        if (!empty($data['iata_code'])) {
            $airport = Airport::query()->where('iata_code', $data['iata_code'])->first();
        }
        if (!$airport && !empty($data['icao_code'])) {
            $airport = Airport::query()->where('icao_code', $data['icao_code'])->first();
        }
    ...
    private function syncTerminals(Airport $airport, array $terminals): void
    {
        foreach ($terminals as $terminal) {
            ...
            if ($code) {
                $existing = $query->where('code', $code)->first();
            } else {
                $existing = $query->where('name', $name)->first();
            }
            ...
            if ($existing) {
                ...
                $airport->terminals()->create($payload);
            }
        }
    }
    ...
    private function locationPayload(array $data, ?int $airportId): array
    {
        return [
            ...
            'code' => $data['code'] ?? ($data['iata_code'] ?? null),
