...
const COPY = {
  en: {
    ...
    actionRequired:"Action Required", actionHint:"Complete these items to improve operational readiness.", noTerminalAction:"Add at least one airport terminal.", noPickupAction:"Add at least one pickup point.", noDropoffAction:"Add at least one dropoff point.", missingMapAction:"Position operational points on the map.", missingMap:"Missing map position", positionNow:"Position now",
  },
  tr: {
    ...
    actionRequired:"Aksiyon Gerekli", actionHint:"Operasyon hazırlığını yükseltmek için bu eksikleri tamamlayın.", noTerminalAction:"En az bir havalimanı terminali ekleyin.", noPickupAction:"En az bir pickup noktası ekleyin.", noDropoffAction:"En az bir dropoff noktası ekleyin.", missingMapAction:"Operasyon noktalarını haritada konumlandırın.", missingMap:"Harita konumu eksik", positionNow:"Şimdi konumlandır",
  },
};
...
export default function ProfessionalLocationOperationsPanel({ initialLocation, language="en", onClose, onChanged }) {
  ...
  const missingMapPoints = useMemo(() => points.filter(point =>
    !hasValidCoordinate(point.latitude) || !hasValidCoordinate(point.longitude)
  ), [points]);

  const readinessActions = useMemo(() => {
    const actions = [];
    if (location.airport && stats.terminals === 0) actions.push({key:"terminal", label:text.noTerminalAction});
    if (stats.pickup === 0) actions.push({key:"pickup", label:text.noPickupAction});
    if (stats.dropoff === 0) actions.push({key:"dropoff", label:text.noDropoffAction});
    if (missingMapPoints.length > 0) actions.push({key:"map", label:`${text.missingMapAction} (${missingMapPoints.length})`});
    return actions;
  }, [location.airport, stats, missingMapPoints.length, text]);
  ...
  function positionPoint(point) {
    editPoint(point);
    window.setTimeout(() => {
      document.querySelector(".lop-map-section")?.scrollIntoView({behavior:"smooth", block:"start"});
    }, 80);
  }
  ...
  return <div className="lop-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}>
    <aside className="lop-panel">
      ...
      <section className="lop-section">
        ...
        {readinessActions.length > 0 && <div className="lop-action-required">
          <div className="lop-action-heading"><div><span>{text.actionRequired}</span><strong>{text.actionHint}</strong></div><b>{readinessActions.length}</b></div>
          <div className="lop-action-list">{readinessActions.map(action=><div key={action.key} className="lop-action-item"><i>!</i><span>{action.label}</span>{action.key==="terminal"&&<button type="button" onClick={addTerminal}>+ {text.addTerminal}</button>}{action.key!=="terminal"&&action.key!=="map"&&<button type="button" onClick={()=>{setEditingPointId(null);setPointForm({...emptyPoint,is_pickup_allowed:action.key==="pickup",is_dropoff_allowed:action.key==="dropoff"});setPointOpen(true);}}>+ {text.addPoint}</button>}</div>)}</div>
          {missingMapPoints.length>0&&<div className="lop-missing-map-list">{missingMapPoints.map(point=><button type="button" key={point.id} onClick={()=>positionPoint(point)}><span><strong>{point.name}</strong><small>{text.missingMap}</small></span><b>{text.positionNow} →</b></button>)}</div>}
        </div>}
        ...
        {points.length ? <div className="lop-point-list">{points.map(point=>{
          const mapMissing = !hasValidCoordinate(point.latitude) || !hasValidCoordinate(point.longitude);
          return <article key={point.id} className={mapMissing?"is-map-missing":""}>
            <div className="lop-point-icon">{point.is_pickup_allowed?"P":point.is_dropoff_allowed?"D":"•"}</div>
            <div className="lop-point-main"><strong>{point.name}</strong><span>{point.point_type?.replaceAll?.("_"," ") || point.point_type}</span><small>{point.airport_terminal?.name || text.noTerminal}</small>{mapMissing&&<em>{text.missingMap}</em>}{point.instructions && <p>{point.instructions}</p>}</div>
            <div className="lop-point-side"><div className="lop-point-meta">{point.is_pickup_allowed&&<b>Pickup</b>}{point.is_dropoff_allowed&&<b>Dropoff</b>}{point.requires_meet_and_greet&&<b>M&G</b>}{point.geofence_radius_meters&&<span>{point.geofence_radius_meters} m</span>}</div><div className="lop-point-actions">{mapMissing&&<button type="button" className="warning" disabled={busy} onClick={()=>positionPoint(point)}>{text.positionNow}</button>}<button type="button" disabled={busy} onClick={()=>editPoint(point)}>{text.edit}</button><button type="button" className="danger" disabled={busy} onClick={()=>removePoint(point)}>{text.remove}</button></div></div>
          </article>;
        })}</div> : !pointOpen && <div className="lop-empty">{text.noPoints}</div>}
      </section>
    </aside>
  </div>;
}
