...
const transferService = {
  ...
  async getTransferEvidences(transferId) {
    if (!transferId) {
      throw new Error(
        "Kanıtları görüntülenecek transfer bulunamadı.",
      );
    }

    const response = await apiClient.get(
      `/transfers/${transferId}/evidences`,
    );
  ...
  async createDispatcherTransfer(
    payload,
  ) {
  ...
  async importDispatcherTransfers(
    file,
  ) {
    if (!file) {
      throw new Error(
        "Aktarılacak Excel dosyası seçilmedi.",
      );
    }
  ...
  if (
    ...
    throw new Error(
      "Toplu atama için en az bir transfer seçilmelidir.",
    );
  }
...
async updateDispatcherTransfer(
    transferId,
    payload,
  ) {
    if (!transferId) {
      throw new Error(
        "Düzenlenecek transfer bulunamadı.",
      );
    }

    const response =
      await apiClient.patch(
        `/dispatcher/transfers/${transferId}`,
        payload,
      );
  ...
  async cancelDispatcherTransfer(
    transferId,
    reason,
  ) {
    if (!transferId) {
      throw new Error(
        "İptal edilecek transfer bulunamadı.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "İptal nedeni en az 10 karakter olmalıdır.",
      );
    }

    const response =
      await apiClient.patch(
        `/dispatcher/transfers/${transferId}/cancel`,
        {
          reason:
            cancellationReason,
        },
