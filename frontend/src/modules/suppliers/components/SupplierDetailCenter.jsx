...
export default function SupplierDetailCenter({ supplierId, drivers = [], vehicles = [], onClose }) {
  ...
  return <div className="supplier-detail-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose?.(); }}>
    <section className="supplier-detail-center" role="dialog" aria-modal="true">
      <header className="supplier-detail-header"><div><small>{text.eyebrow}</small><h2>{supplier?.company_name || "Supplier"}</h2><p>{formatLocation(supplier) || text.noData}</p></div><div className="supplier-detail-header-side">{supplier && <span className={`supplier-detail-operation ${supplier.is_active ? "is-open" : "is-closed"}`}>{supplier.is_active ? text.operational : text.closed}</span>}<button type="button" onClick={onClose}>× {text.close}</button></div></header>
...
function Operations({ rows, text, language }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((item) => {
      ...
      if (!q) return true;
      const haystack = [item.booking_reference,item.ota_booking_reference,item.passenger_name,item.passenger_phone,item.flight_number,item.flight_code,item.pickup_address,item.dropoff_address,item.pickup_location?.name,item.dropoff_location?.name].filter(Boolean).join(" ").toLocaleLowerCase();
  ...
  return <div className="supplier-operation-workspace">
    <div className="supplier-operation-filters">
      ...
      <select value={status} onChange={(e)=>setStatus(e.target.value)}><option value="">{text.allStatuses}</option>{["pending","accepted","on_the_way","arrived","passenger_called","passenger_on_board","trip_started","completed","no_show","cancelled"].map((value)=><option key={value} value={value}>{getTransferStatusLabel(value,text)}</option>)}</select>
...
function Finance({ supplier, transfers, finance, text }) { return <div className="supplier-detail-stack"><div className="supplier-detail-kpis"><MiniKpi label={text.totalTransfers} value={transfers}/><MiniKpi label={text.activeTransfers} value={finance.active}/><MiniKpi label={text.completed} value={finance.completed}/><MiniKpi label={text.recordedSales} value={`${finance.sales.toFixed(2)} ${supplier.default_currency || ""}`}/></div><div className="supplier-detail-note">{text.financeNote}</div></div>; }
function Documents({ supplier, text }) { const branches=Array.isArray(supplier.branches)?supplier.branches:[]; return <div className="supplier-detail-stack"><section className="supplier-detail-section"><h3>{text.branchesTitle}</h3>{branches.length?<div className="supplier-branch-grid">{branches.map(item=><article key={item.id}><strong>{item.name||item.city||"Branch"}</strong><span>{[item.city,item.country_name||item.country_code].filter(Boolean).join(", ")||item.address||"—"}</span></article>)}</div>:<div className="supplier-detail-state">{text.noData}</div>}</section><section className="supplier-detail-section"><h3>{text.complianceTitle}</h3><div className="supplier-detail-note">{text.notConnected}</div></section></div>; }
function Activity({ supplier, text, language }) { const rows=Array.isArray(supplier.approval_logs)?supplier.approval_logs:[]; if(!rows.length)return <div className="supplier-detail-state">{text.noActivity}</div>; return <div className="supplier-detail-table-wrap"><table><thead><tr><th>{text.date}</th><th>{text.action}</th><th>{text.status}</th><th>{text.changedBy}</th><th>{text.note}</th></tr></thead><tbody>{rows.map(item=><tr key={item.id}><td>{formatDate(item.created_at,language,true)}</td><td>{item.action||"—"}</td><td>{item.old_status&&item.new_status?`${item.old_status} → ${item.new_status}`:item.new_status||"—"}</td><td>{item.changed_by?.name||item.changedBy?.name||"—"}</td><td>{item.note||item.reason||"—"}</td></tr>)}</tbody></table></div>; }
