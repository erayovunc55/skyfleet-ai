...
class OperationalAlertController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $alerts = collect()
            ->concat(
                $this->upcomingUnassigned(
                    $now
                )
            )
            ->concat(
                $this->overdueTransfers(
                    $now
                )
            )
            ->concat(
    ...
    public function markRead(
        ...
        foreach (
            array_unique(
                $validated['alert_keys']
            ) as $alertKey
        ) {
            OperationalAlertRead::query()
                ->updateOrCreate(
                    [
                        'user_id' =>
                            $request->user()->id,
    ...
    private function upcomingUnassigned(
        Carbon $now
    ): Collection {
                ...
                fn (Transfer $transfer): array =>
                    $this->makeAlert(
                        key: 'unassigned:' .
                            $transfer->id,
                        level: 'critical',
                        title: 'Yaklaşan sürücüsüz transfer',
                        message: sprintf(
                            '%s rezervasyonuna sürücü atanmadı.',
                            $transfer
                                ->booking_reference
                        ),
                        transfer: $transfer,
    ...
    private function overdueTransfers(
        Carbon $now
    ): Collection {
        return Transfer::query()
            ->whereBetween(
    ...
    private function recentNoShowEvidence(
        Carbon $now
    ): Collection {
        return TransferEvidence::query()
            ...
            ->get()
            ->filter(
                fn (TransferEvidence $evidence): bool =>
                    (bool) $evidence->transfer
            )
            ->map(
                fn (TransferEvidence $evidence): array =>
                    $this->makeAlert(
                        key: 'evidence:' .
                            $evidence->id,
                        level: 'info',
    ...
    private function recentCancellations(
        Carbon $now
    ): Collection {
        return Transfer::query()
